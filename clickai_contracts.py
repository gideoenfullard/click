...
def _letter_body(d,ss,money):
    dt=d.get("doc_type","");g=lambda k:ss(str(d.get(k,""))) if d.get(k) else "";nl=lambda t:t.replace("\n","<br>") if t else ""
    ...
    if dt=="written_warning":
        ...
        return f'<p {p}>You are hereby notified to attend a <strong>Disciplinary Hearing</strong>:</p><table style="width:100%;font-size:12px;margin:15px 0;"><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;width:150px;">Date:</td><td style="padding:8px 0;font-weight:600;">{g("hearing_date")}</td></tr><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;">Time:</td><td style="padding:8px 0;">{g("hearing_time")}</td></tr><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;">Venue:</td><td style="padding:8px 0;">{g("hearing_venue")}</td></tr></table><h3 {h}>Charges</h3><p {p}>{nl(g("charges"))}</p>{f"<h3 {h}>Witnesses</h3><p {p}>{nl(g(chr(119)+'itnesses'))}</p>" if g("witnesses") else ""}<div style="background:#eff6ff;border-left:4px solid #3b82f6;padding:10px 14px;border-radius:6px;margin-top:15px;"><p style="margin:0;font-size:11px;"><strong>YOUR RIGHTS:</strong> Representation by colleague/shop steward; call witnesses; cross-examine; present mitigation. Non-attendance without valid reason → hearing proceeds in absence.</p></div>'
    elif dt=="suspension":
        pw="with full pay" if str(d.get("with_pay","")).lower() in ("yes","true","1") else "without pay"
        end_row = f'<tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;">End:</td><td style="padding:8px 0;">{g("suspension_end")}</td></tr>' if g("suspension_end") else ""
        return f'<p {p}>You are hereby suspended <strong>{pw}</strong> with immediate effect.</p><h3 {h}>Reason</h3><p {p}>{nl(g("reason"))}</p><table style="width:100%;font-size:12px;margin:15px 0;"><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;width:180px;">Start:</td><td style="padding:8px 0;font-weight:600;">{g("suspension_start")}</td></tr>{end_row}</table><h3 {h}>Conditions</h3><p {p}>{nl(g("conditions"))}</p><div style="background:#f5f3ff;border-left:4px solid #8b5cf6;padding:10px 14px;border-radius:6px;margin-top:15px;"><p style="margin:0;font-size:11px;">Precautionary measure — does not imply guilt. LRA rights preserved.</p></div>'
    elif dt=="hearing_outcome":
        return f'<h3 {h}>Charges</h3><p {p}>{nl(g("charges"))}</p><h3 {h}>Finding</h3><p {p}>{nl(g("finding"))}</p><h3 {h}>Sanction</h3><p {p} style="font-weight:700;">{nl(g("sanction"))}</p>{f"<h3 {h}>Mitigating</h3><p {p}>{nl(g(chr(109)+'itigating_factors'))}</p>" if g("mitigating_factors") else ""}{f"<h3 {h}>Aggravating</h3><p {p}>{nl(g(chr(97)+'ggravating_factors'))}</p>" if g("aggravating_factors") else ""}<h3 {h}>Appeal</h3><p {p}>{nl(g("appeal_rights"))}</p>'
    elif dt=="dismissal":
        return f'<p {p}>Your employment is <strong>terminated with immediate effect</strong>.</p><h3 {h}>Reason</h3><p {p}>{nl(g("reason"))}</p><table style="width:100%;font-size:12px;margin:15px 0;"><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;width:180px;">Effective date:</td><td style="padding:8px 0;font-weight:600;">{g("effective_date")}</td></tr></table>{f"<h3 {h}>Final Pay</h3><p {p}>{nl(g(chr(102)+'inal_pay_details'))}</p>" if g("final_pay_details") else ""}{f"<h3 {h}>Return Property</h3><p {p}>{nl(g(chr(114)+'eturn_property'))}</p>" if g("return_property") else ""}<h3 {h}>Appeal</h3><p {p}>{nl(g("appeal_rights"))}</p><div style="background:#fef2f2;border-left:4px solid #dc2626;padding:10px 14px;border-radius:6px;margin-top:15px;"><p style="margin:0;font-size:11px;">CCMA referral within 30 days (Section 191, LRA 66 of 1995).</p></div>'
    elif dt=="resignation_acceptance":
        ns="with" if str(d.get("notice_served","")).lower() in ("yes","true","1") else "without"
        return f'<p {p}>We acknowledge your resignation dated <strong>{g("resignation_date")}</strong>, accepted {ns} notice.</p><p {p}>Last working day: <strong>{g("last_working_day")}</strong></p>{f"<h3 {h}>Handover</h3><p {p}>{nl(g(chr(104)+'andover_requirements'))}</p>" if g("handover_requirements") else ""}{f"<h3 {h}>Final Pay</h3><p {p}>{nl(g(chr(102)+'inal_pay_details'))}</p>" if g("final_pay_details") else ""}<p {p}>Thank you for your service. We wish you well.</p>'
    elif dt=="promotion":
        dep_row = f'<tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;">Department:</td><td style="padding:8px 0;">{g("new_department")}</td></tr>' if g("new_department") else ""
        sal_row = f'<tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;">New salary:</td><td style="padding:8px 0;font-weight:600;">R{float(d.get("new_salary",0)):,.2f}/month</td></tr>' if d.get("new_salary") else ""
        resp_html = f"<h3 {h}>Responsibilities</h3><p {p}>{nl(g('new_responsibilities'))}</p>" if g("new_responsibilities") else ""
        return f'<p {p}>We are pleased to confirm your promotion, effective <strong>{g("effective_date")}</strong>.</p><table style="width:100%;font-size:12px;margin:15px 0;"><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;width:180px;">New position:</td><td style="padding:8px 0;font-weight:600;">{g("new_position")}</td></tr>{dep_row}{sal_row}</table>{resp_html}<p {p}>Congratulations. All other terms remain unchanged.</p>'
    elif dt=="salary_increase":
        cur=float(d.get("current_salary",0));nw=float(d.get("new_salary",0));pct=round((nw-cur)/cur*100,1) if cur>0 else 0
        return f'<p {p}>Your salary has been revised effective <strong>{g("effective_date")}</strong>.</p><table style="width:100%;font-size:12px;margin:15px 0;"><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;width:180px;">Current:</td><td style="padding:8px 0;">R{cur:,.2f}/month</td></tr><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;">New:</td><td style="padding:8px 0;font-weight:700;color:#10b981;">R{nw:,.2f}/month</td></tr><tr style="border-bottom:1px solid #e5e7eb;"><td style="padding:8px 0;color:#666;">Increase:</td><td style="padding:8px 0;font-weight:600;">{pct}%</td></tr></table>{f"<p {p}><strong>Reason:</strong> {nl(g(chr(105)+'ncrease_reason'))}</p>" if g("increase_reason") else ""}<p {p}>All other terms unchanged.</p>'
    return ""
