...
def register_invoicing_routes(app, db, login_required, Auth, render_page,
    ...
    def invoice_view(invoice_id):
        ...
        for item in items:
            ...
            # POS sale-campaign / manual discounts are stored as discount_pct
            ...
            <div style="padding:26px 25px 15px;display:flex;justify-content:space-between;align-items:flex-end;">
                ...
                <div style="font-size:12px;color:#666;max-width:55%;">
                    ...
                    <div style="margin-top:12px;font-size:11px;color:#999;">
                        {f'<div style="margin-bottom:8px;padding:8px 12px;border:3px double #000;font-size:14px;font-weight:bold;color:#000;display:inline-block;">*** SALE! YOU SAVED R{_inv_saved_total:.2f} ***</div>' if _inv_saved_total > 0.005 else ''}
                ...
                <table style="width:220px;border-collapse:collapse;">
                    <tr style="border-bottom:1px solid #e5e7eb;">
                        ...
                        <td style="padding:4px 8px;text-align:right;color:#333;font-size:11px;">{money(_inv_saved_total)}</td>
                    </tr>
                    <tr style="border-bottom:1px solid #e5e7eb;">
        ...
        try:
            ...
            try:
                ...
                _inv_doc = {
                    ...
                    "date": date,
                    "delivery_note": invoice.get("delivery_note", "") or invoice.get("delivery_note_number", ""),
                    "payment_terms": invoice.get("payment_terms", ""),
                    "reference": invoice.get("reference", "") or invoice.get("order_reference", ""),
                    "sales_person": invoice.get("sales_person", "") or invoice.get("salesman_name", "") or invoice.get("sales_rep", ""),
                    "subtotal": subtotal,
                    "vat": vat,
                    "total": total,
                    "items": items,
