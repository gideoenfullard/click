    ...
    def supplier_view(supplier_id):
        ...
        if can_see_balances:
            for si in supplier_invoices[:200]:
                ...
                _si_num = si.get("invoice_number", "-")
                _si_linked_po = _si_to_po.get(_si_num, "")
                _si_po_html = f'<span style="font-size:11px;color:var(--primary);">{_si_linked_po}</span>' if _si_linked_po else '<span style="font-size:11px;color:var(--text-muted);">-</span>'
                supplier_inv_html += f'''
                <tr style="cursor:pointer;" onclick="window.location='/supplier-invoice/{si.get("id")}'">
                    <td>{safe_string(_si_num)}</td>
                    <td>{si.get("date", "-")}</td>
        ...
        # ── CROSS-REFERENCE MAP: Link POs ↔ Supplier Invoices ──
        # Build lookup: PO number → SI numbers, and SI → PO number
        _po_to_si = {}   # po_number → [si_numbers]
        _si_to_po = {}   # si invoice_number → po_number
        for si in supplier_invoices:
            si_notes = (si.get("notes") or "").strip()
            si_num = si.get("invoice_number", "")
            # Match via notes field: "From PO: PO-00259"
            if "From PO:" in si_notes:
                _linked_po = si_notes.split("From PO:")[-1].strip().split()[0] if si_notes else ""
                if _linked_po:
                    _po_to_si.setdefault(_linked_po, []).append(si_num)
                    _si_to_po[si_num] = _linked_po
            # Also try matching by PO number embedded in SI number (e.g. SI-00259 from PO-00259)
            if not _si_to_po.get(si_num):
                _si_suffix = si_num.replace("SI-", "").replace("SINV-", "").strip()
                for _po in purchase_orders:
                    _po_suffix = _po.get("po_number", "").replace("PO-", "").strip()
                    if _po_suffix and _si_suffix and _po_suffix == _si_suffix:
                        _po_num = _po.get("po_number", "")
                        _po_to_si.setdefault(_po_num, []).append(si_num)
                        _si_to_po[si_num] = _po_num
                        break
        ...
        for po in purchase_orders[:200]:
            ...
            _po_num = po.get("po_number", "-")
            _linked_si_list = _po_to_si.get(_po_num, [])
            _linked_si_html = ""
            if _linked_si_list:
                _si_labels = ", ".join(_linked_si_list[:3])
                _linked_si_html = f'<span style="font-size:11px;color:var(--primary);">{_si_labels}</span>'
            else:
                _linked_si_html = '<span style="font-size:11px;color:var(--text-muted);">-</span>'
            po_html += f'''
            <tr style="cursor:pointer;" onclick="window.location='/purchase/{po.get("id")}'">
                <td>{_po_num}</td>
                <td>{po.get("date", "-")}</td>
                <td>{money(po.get("total", 0))}</td>
                <td>{_linked_si_html}</td>
        ...
        <div class="card" style="margin-top:20px;">
            ...
            <table class="table" id="supplierInvTable">
                <thead>
                    <tr><th>Invoice #</th><th>Date</th><th>Due Date</th><th>Amount</th><th>PO</th><th>Status</th></tr>
                </thead>
                <tbody>
                    {supplier_inv_html or "<tr><td colspan='6' style='text-align:center;color:var(--text-muted);'>No invoices</td></tr>"}
                </tbody>
            </table>
        </div>
        ''' if can_see_balances else '') + f'''
        ...
        <div class="card" style="margin-top:20px;">
            ...
            <table class="table" id="poTable">
                <thead>
                    <tr><th>PO Number</th><th>Date</th><th>Total</th><th>Invoice</th><th>Status</th></tr>
                </thead>
                <tbody>
                    {po_html if po_html else "<tr><td colspan='5' style='text-align:center;color:var(--text-muted)'>No purchase orders</td></tr>"}
                </tbody>
            </table>
        </div>
